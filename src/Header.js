import React, { useEffect } from "react";
import "./Header.css";
import { Link } from "react-router-dom";

import { useDispatch, useSelector } from "react-redux";
import { getProducts } from "../src/store/actions/products";
import { getCategories } from "../src/store/actions/category";

// import SearchIcon from "@material-ui/icons/Search";
import SearchIcon from "@mui/icons-material/Search";
import ShoppingCartIcon from "@mui/icons-material/ShoppingCart";
import LocationOnIcon from "@mui/icons-material/LocationOn";
function Header() {
  const dispatch = useDispatch();
  const { isLoading, categories } = useSelector((state) => state.category);

  useEffect(() => {
    dispatch(getProducts());
    dispatch(getCategories());
  }, []);

  return (
    <div className="sticky top-0 z-50">
      <div className=" sticky top-0 z-50 text-center header flex items-center p-1 pl-1  justify-between text-white bg-[#232f3e]">
        <img
          className="header_logo h-16"
          src="data:image/png;base64,"
          alt="logo nav bar"
        ></img>
        <div className="flex items-center">
          <LocationOnIcon />
          <div className="header_address text-left">
            <div className="header_optionLineOne">Hello </div>
            <div className=" font-bold header_optionLineTwo">
              Select Your Address
            </div>
          </div>
        </div>

        <div className="header_search pr-2 flex w-[50%] items-center bg-[#f1a644] rounded-md">
          <input
            className="header_searchInput mr-1 flex-1 h-[35px]"
            type="text"
          />
          {/* {logo} */}
          <SearchIcon className="header_searchIcon " />
        </div>

        <div className="header_nav flex gap-5">
          <div className="header_option">
            <div className="header_optionLineOne">Hello, Sign in</div>
            <div className="header_optionLineTwo font-bold ">
              Account &amp; Lists
            </div>
          </div>

          <div className="header_option">
            <div className="header_optionLineOne">Returns</div>
            <div className="header_optionLineTwo font-bold ">&amp; Orders</div>
          </div>
          <div className="header_option flex items-center pl-2 mr-2">
            <Link to={`/cart`} className="py-1">
              <ShoppingCartIcon className="h-64" /> Cart
            </Link>
          </div>
        </div>
      </div>
      <div>
        <div className="sticky z-50 flex   px-[120px] justify-between text-white bg-[#232f3e]">
          {categories.map(({ _id, name }) => (
            <Link key={_id} to={`/categories/${_id}`} className="py-1">
              {name}
            </Link>
          ))}
        </div>
      </div>
    </div>
  );
}

export default Header;
